...
@pytest.mark.asyncio
async def test_search_with_cache(load_bulk_data_to_persons_es) -> None:
    """
    ...
    logger.info(f"Успешно загружено {len(es_data)} фильмов в индекс {config.es_persons_index_mapping}.")

    # Создаем HTTP-сессию
    async with aiohttp.ClientSession() as session:
        # Первый запрос (предварительное заполнение кэша)
        start = time.monotonic()
        url = f"{config.service_url}/api/v1/persons/persons_by_full_name"
