...
async def test_search_by_genre(es_client, load_bulk_data_to_es, query_data: dict) -> None:
    """
    ...
    try:
        ...
        logger.info(f"Успешно загружено {len(es_data)} фильмов в индекс {config.elastic_index}.")

        # Создаем HTTP-сессию для запроса к API
        async with aiohttp.ClientSession() as session:
            url = f"{config.service_url}/api/v1/search/films_by_genre"
