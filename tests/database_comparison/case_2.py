...
def user_like_rating_films_by_film(pg_uow: PGUOW, mongo_uow: MongoUOW, film_id: str, min_: int = 7):
    ...
    if select_pg_len != select_mongo_len:
        logger.warning(
            f"Case_2(user_like_rating_films_by_film): количество select-данных не совпадает: "
            f"PG({select_pg_len}), Mongo({select_mongo_len})"
        )

    if select_time_pg < select_time_mongo:
        logger.info(
            f"Case_2(user_like_rating_films_by_film): "
