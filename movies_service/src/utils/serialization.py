...
def model_to_dict(model: Any) -> dict:
    ...
    Если объект является экземпляром:
      ...
      - содержит типы, не сериализуемые по умолчанию (Decimal, UUID), то они
      преобразуются к строке.
    ...
    # Если это экземпляр Pydantic BaseModel, можно использовать метод dict()
        ...
        data = model.dict()
    # Если объект не поддерживает ни один из методов, пробуем взять его
    # __dict__
    elif hasattr(model, "__dict__"):
        data = model.__dict__
    else:
        raise TypeError(
            f"Объект типа {model.__class__.__name__} не может быть "
