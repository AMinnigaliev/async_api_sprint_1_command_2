...
from models.events.element_click import ClickElementDC, ClickElementPlayMovies, ClickElementsTypesEnum
...
class ElementClickEventRule(AbstractEventRule):
    ...
    def click_element_dc(self) -> ClickElementDC:
        if not self._click_element_dc:
            data = {
                ...
                "element_type": self.event_value["data"].get("element_type") if self.event_value.get("data") else None,
            }
            data.update({key_: value_ for key_, value_ in self.event_value.get("data", {}).items()})
    ...
    def event_storage_key(self) -> str:
        return f"{self.event_key}:{self.click_element_dc.element_type}:{self.click_element_dc.user_id}:{uuid.uuid4()}"

    @property
    def element_type_executors(self) -> dict:
        return {
            ClickElementsTypesEnum.play_movies_element.name: self._click_element_play_movies,
        }

    async def execute(self) -> tuple:
        event_storage_key, event_data = None, None

        if element_type_executor := self.element_type_executors.get(self.click_element_dc.element_type):
            try:
                if iscoroutinefunction(element_type_executor):
                    event_storage_key, event_data = await element_type_executor()

                else:
                    event_storage_key, event_data = element_type_executor()

            except EventRuleError as ex:
                logger.warning(f"ElementClickEvent: error(element_type={self.click_element_dc.element_type}): {ex}")

        else:
            logger.warning(
                f"ElementClickEvent: not found executor for element_type='{self.click_element_dc.element_type}'"
            )
